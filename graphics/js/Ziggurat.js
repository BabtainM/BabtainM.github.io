...
function createScene() {
    n = controls.n;
    zig = ziggurat(controls.n, controls.nbrSides, controls.height, controls.scale, controls.opacity);
...
function ziggurat(n, nbrSides, height, sfactor, opacity, materials) {
    let radius = 1.0;
    let ypos = height / 2.0;
    let sf = sfactor;
    if (!materials) mats = [];
    ...
    for (let i = 0; i < n ; i++) {
        ...
        let mat;
        if (!materials) {
            let matArgs = {transparent: true, opacity: opacity, color:'blue'};
...
function initGui() {
    var gui = new dat.GUI();
    gui.add(controls, 'n', 1, 80).step(1).onChange(update);
...
function update() {
    ...
    if (zig)
        scene.remove(zig);
    if (n == controls.n) {// use current materials mats
        zig = ziggurat(n, nbrSides, height, scale, opacity, mats);
    } else {
        n = controls.n;
        zig = ziggurat(n, nbrSides, height, scale, opacity);
    }
